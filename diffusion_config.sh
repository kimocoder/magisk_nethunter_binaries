...
INST_NAME="NetHunter Binaries installer script";
AUTH_NAME="kimocoder @ aircrack-ng.org";
...
custom_install() {
  ui_print " ";
  ui_print "Installing binaries to $BIN ...";
...
custom_exitmsg() {
  if ! $BOOTMODE && [ "$ACTION" == installation ]; then
    ui_print " ";
    ui_print "binaries may now be run temporarily from";
